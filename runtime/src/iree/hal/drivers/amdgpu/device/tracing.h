...
// Enables IREE_OCL_TRACE_* macros for instrumented tracing.
...
// is loaded onto the GPU agent. The host tracing infra creates a shadow copy
// of the code in host memory and adjusts the address here into that shadow.
//
...
// modifying them, allowing for host pointers to be round-tripped from the host.
// In this way calling a pointer a iree_hal_amdgpu_trace_rodata_ptr_t really
...
// can access it.
typedef struct iree_hal_amdgpu_trace_src_loc_s {
  const char* name;
  const char* function;
  const char* file;
  uint32_t line;
...
enum {
  ...
  IREE_HAL_AMDGPU_TRACE_EVENT_EXECUTION_ZONE_NOTIFY,
...
typedef struct IREE_OCL_ALIGNAS(8) iree_hal_amdgpu_trace_zone_begin_s {
...
typedef struct IREE_OCL_ALIGNAS(8) iree_hal_amdgpu_trace_zone_end_s {
  // IREE_HAL_AMDGPU_TRACE_EVENT_ZONE_END
  iree_hal_amdgpu_trace_event_type_t event_type;
  uint8_t reserved[7];  // may be uninitialized
...
typedef struct IREE_OCL_ALIGNAS(8) iree_hal_amdgpu_trace_zone_value_i64_s {
  // IREE_HAL_AMDGPU_TRACE_EVENT_ZONE_VALUE_I64
  iree_hal_amdgpu_trace_event_type_t event_type;
  uint8_t reserved[7];  // may be uninitialized
...
typedef struct IREE_OCL_ALIGNAS(8)
    iree_hal_amdgpu_trace_zone_value_text_literal_s {
  // IREE_HAL_AMDGPU_TRACE_EVENT_ZONE_VALUE_TEXT_LITERAL
  iree_hal_amdgpu_trace_event_type_t event_type;
  uint8_t reserved[3];
  // Payload value attached to the zone.
...
// generated values.
typedef struct IREE_OCL_ALIGNAS(8)
    iree_hal_amdgpu_trace_zone_value_text_dynamic_s {
  // IREE_HAL_AMDGPU_TRACE_EVENT_ZONE_VALUE_TEXT_DYNAMIC
  iree_hal_amdgpu_trace_event_type_t event_type;
  uint8_t reserved[3];
...
typedef struct IREE_OCL_ALIGNAS(8)
    iree_hal_amdgpu_trace_execution_zone_begin_s {
  // IREE_HAL_AMDGPU_TRACE_EVENT_EXECUTION_ZONE_BEGIN
  iree_hal_amdgpu_trace_event_type_t event_type;
...
typedef struct IREE_OCL_ALIGNAS(8) iree_hal_amdgpu_trace_execution_zone_end_s {
  // IREE_HAL_AMDGPU_TRACE_EVENT_EXECUTION_ZONE_END
  iree_hal_amdgpu_trace_event_type_t event_type;
  // Execution trace ID used to distinguish different execution units. This is
  // assigned on the host when the execution context is configured.
...
enum iree_hal_amdgpu_trace_execution_zone_type_e {
  ...
  // Indicates a DMA copy operation. export_loc is ignored.
  IREE_HAL_AMDGPU_TRACE_EXECUTION_ZONE_TYPE_COPY,
  // Indicates a DMA fill operation. export_loc is ignored.
  IREE_HAL_AMDGPU_TRACE_EXECUTION_ZONE_TYPE_FILL,
  // Indicates an internal bookkeeping dispatch. export_loc is ignored.
...
typedef struct IREE_OCL_ALIGNAS(8)
    iree_hal_amdgpu_trace_execution_zone_dispatch_s {
  // IREE_HAL_AMDGPU_TRACE_EVENT_EXECUTION_ZONE_DISPATCH
  iree_hal_amdgpu_trace_event_type_t event_type;
...
// Notifies the trace sink of a single completed query.
typedef struct IREE_OCL_ALIGNAS(8)
    iree_hal_amdgpu_trace_execution_zone_notify_s {
  // IREE_HAL_AMDGPU_TRACE_EVENT_EXECUTION_ZONE_NOTIFY
  iree_hal_amdgpu_trace_event_type_t event_type;
  // Execution trace ID used to distinguish different execution units. This is
  // assigned on the host when the execution context is configured.
  iree_hal_amdgpu_trace_executor_id_t executor_id;
  uint8_t reserved0[2];  // may be uninitialized
  // A query ID being notified.
  iree_hal_amdgpu_trace_execution_query_id_t execution_query_id;
  uint8_t reserved1[2];  // may be uninitialized
  // Timestamp of the query as executed.
  iree_hal_amdgpu_trace_agent_timestamp_t execution_timestamp;
} iree_hal_amdgpu_trace_execution_zone_notify_t;
...
typedef struct IREE_OCL_ALIGNAS(8)
    iree_hal_amdgpu_trace_execution_zone_notify_batch_s {
  // IREE_HAL_AMDGPU_TRACE_EVENT_EXECUTION_ZONE_NOTIFY_BATCH
  iree_hal_amdgpu_trace_event_type_t event_type;
  // Execution trace ID used to distinguish different execution units. This is
  // assigned on the host when the execution context is configured.
  ...
  // Timestamp of the query as executed.
  iree_hal_amdgpu_trace_agent_timestamp_t
      execution_timestamps[/*execution_query_count*/];
} iree_hal_amdgpu_trace_execution_zone_notify_batch_t;

// Records the allocation of a block of memory from a named pool.
typedef struct IREE_OCL_ALIGNAS(8) iree_hal_amdgpu_trace_memory_alloc_s {
  // IREE_HAL_AMDGPU_TRACE_EVENT_MEMORY_ALLOC
  iree_hal_amdgpu_trace_event_type_t event_type;
...
typedef struct IREE_OCL_ALIGNAS(8) iree_hal_amdgpu_trace_memory_free_s {
  // IREE_HAL_AMDGPU_TRACE_EVENT_MEMORY_FREE
  iree_hal_amdgpu_trace_event_type_t event_type;
  uint8_t reserved[7];
  // Pool name used as both a title for the pool and the unique ID for
...
typedef struct IREE_OCL_ALIGNAS(8) iree_hal_amdgpu_trace_message_literal_s {
  // IREE_HAL_AMDGPU_TRACE_EVENT_MESSAGE_LITERAL
  iree_hal_amdgpu_trace_event_type_t event_type;
...
// The contents are embedded in the trace buffer to support dynamically
// generated values.
typedef struct IREE_OCL_ALIGNAS(8) iree_hal_amdgpu_trace_message_dynamic_s {
  // IREE_HAL_AMDGPU_TRACE_EVENT_MESSAGE_DYNAMIC
  iree_hal_amdgpu_trace_event_type_t event_type;
  uint8_t reserved[3];
  // Length of the value in characters.
...
typedef struct IREE_OCL_ALIGNAS(8) iree_hal_amdgpu_trace_plot_config_s {
  // IREE_HAL_AMDGPU_TRACE_EVENT_PLOT_CONFIG
  iree_hal_amdgpu_trace_event_type_t event_type;
...
typedef struct IREE_OCL_ALIGNAS(8) iree_hal_amdgpu_trace_plot_value_i64_s {
  // IREE_HAL_AMDGPU_TRACE_EVENT_PLOT_VALUE_I64
  iree_hal_amdgpu_trace_event_type_t event_type;
...
#define IREE_HAL_AMDGPU_DEVICE_QUERY_RINGBUFFER_CAPACITY 0xFFFF
...
typedef struct iree_hal_amdgpu_device_query_ringbuffer_s {
  uint16_t capacity;
  uint64_t read_index;
...
// Initializes the signals in a query ringbuffer.
...
// index.
static inline IREE_OCL_ATTRIBUTE_ALWAYS_INLINE
    iree_hal_amdgpu_trace_execution_query_id_t
    iree_hal_amdgpu_device_query_ringbuffer_query_id(
        IREE_OCL_GLOBAL const iree_hal_amdgpu_device_query_ringbuffer_t*
...
static inline IREE_OCL_ATTRIBUTE_ALWAYS_INLINE iree_hsa_signal_t
...
typedef struct iree_hal_amdgpu_device_trace_buffer_s {
  ...
  IREE_OCL_GLOBAL uint8_t* IREE_OCL_RESTRICT ringbuffer_base;
  ...
  uint64_t ringbuffer_physical_ptr;
  ...
  iree_hal_amdgpu_device_atomic_uint64_t read_commit_offset;
  ...
  // Absolute offset - must be masked with ringbuffer_mask.
  IREE_OCL_ALIGNAS(iree_hal_amdgpu_device_destructive_interference_size)
  iree_hal_amdgpu_device_atomic_uint64_t write_reserve_offset;
  ...
  // Absolute offset - must be masked with ringbuffer_mask.
  IREE_OCL_ALIGNAS(iree_hal_amdgpu_device_destructive_interference_size)
  iree_hal_amdgpu_device_atomic_uint64_t write_commit_offset;
...
typedef uint32_t iree_ocl_zone_id_t;

#if defined(IREE_OCL_TARGET_DEVICE)

// Colors used for messages based on the level provided to the macro.
enum {
  IREE_OCL_TRACE_MESSAGE_LEVEL_ERROR = 0xFF0000u,
  IREE_OCL_TRACE_MESSAGE_LEVEL_WARNING = 0xFFFF00u,
  IREE_OCL_TRACE_MESSAGE_LEVEL_INFO = 0xFFFFFFu,
  IREE_OCL_TRACE_MESSAGE_LEVEL_VERBOSE = 0xC0C0C0u,
  IREE_OCL_TRACE_MESSAGE_LEVEL_DEBUG = 0x00FF00u,
};

#if IREE_HAL_AMDGPU_TRACING_FEATURES

#define IREE_OCL_TRACE_BUFFER_DEFINE(trace_buffer) \
  __iree_ocl_trace_buffer = (trace_buffer)
#define IREE_OCL_TRACE_BUFFER() (__iree_ocl_trace_buffer)
...
#define IREE_OCL_TRACE_CONCAT_(x, y) IREE_OCL_TRACE_CONCAT_INDIRECT_(x, y)
#define IREE_OCL_TRACE_CONCAT_INDIRECT_(x, y) x##y

// Begins a new zone with the parent function name.
#define IREE_OCL_TRACE_ZONE_BEGIN(zone_id) \
  IREE_OCL_TRACE_ZONE_BEGIN_NAMED(zone_id, NULL)
...
// reference.
#define IREE_OCL_TRACE_ZONE_BEGIN_NAMED(zone_id, name_literal) \
  IREE_OCL_TRACE_ZONE_BEGIN_NAMED_COLORED(zone_id, name_literal, 0)

// Begins a new zone with the given compile-time |name_literal| and color.
// The literal must be static const and will be embedded in the trace buffer by
// reference.
#define IREE_OCL_TRACE_ZONE_BEGIN_NAMED_COLORED(zone_id, name_literal, color) \
  static const iree_hal_amdgpu_trace_src_loc_t IREE_OCL_TRACE_CONCAT_(        \
      __iree_ocl_trace_src_loc, __LINE__) = {                                 \
      (name_literal), __FUNCTION__, __FILE__, (uint32_t)__LINE__, (color),    \
  };                                                                          \
  iree_ocl_zone_id_t zone_id = iree_hal_amdgpu_device_trace_zone_begin(       \
      IREE_OCL_TRACE_BUFFER(),                                                \
      (iree_hal_amdgpu_trace_src_loc_ptr_t) &                                 \
          IREE_OCL_TRACE_CONCAT_(__iree_ocl_trace_src_loc, __LINE__));

// Ends the current zone. Must be passed the |zone_id| from the _BEGIN.
#define IREE_OCL_TRACE_ZONE_END(zone_id) \
  iree_hal_amdgpu_device_trace_zone_end(IREE_OCL_TRACE_BUFFER())

// Appends an int64_t value to the parent zone. May be called multiple times.
#define IREE_OCL_TRACE_ZONE_APPEND_VALUE_I64(zone_id, value)                  \
  iree_hal_amdgpu_device_trace_zone_append_value_i64(IREE_OCL_TRACE_BUFFER(), \
                                                     (int64_t)(value))
...
#define IREE_OCL_TRACE_ZONE_APPEND_TEXT_LITERAL(zone_id, value_literal) \
...
// trace buffer.
#define IREE_OCL_TRACE_ZONE_APPEND_TEXT_DYNAMIC(...)                     \
  IREE_OCL_TRACE_IMPL_GET_VARIADIC_(                                     \
      (__VA_ARGS__, IREE_OCL_TRACE_ZONE_APPEND_TEXT_STRING_VIEW_DYNAMIC, \
       IREE_OCL_TRACE_ZONE_APPEND_TEXT_CSTRING_DYNAMIC))                 \
  (__VA_ARGS__)
#define IREE_OCL_TRACE_ZONE_APPEND_TEXT_CSTRING_DYNAMIC(zone_id, value)   \
  IREE_OCL_TRACE_ZONE_APPEND_TEXT_STRING_VIEW_DYNAMIC((zone_id), (value), \
                                                      strlen(value))
#define IREE_OCL_TRACE_ZONE_APPEND_TEXT_STRING_VIEW_DYNAMIC(zone_id, value, \
                                                            value_length)   \
  iree_hal_amdgpu_device_trace_zone_append_text_dynamic(                    \
      IREE_OCL_TRACE_BUFFER(), (value), (value_length))
...
#define IREE_OCL_TRACE_PLOT_CONFIGURE(name_literal, type, flags, color)   \
...
#define IREE_OCL_TRACE_PLOT_VALUE_I64(name_literal, value) \
  iree_hal_amdgpu_device_trace_plot_value_i64(             \
      IREE_OCL_TRACE_BUFFER(),                             \
      (iree_hal_amdgpu_trace_string_literal_ptr_t)(name_literal), (value))

// Utilities:
#define IREE_OCL_TRACE_IMPL_GET_VARIADIC_HELPER_(_1, _2, _3, NAME, ...) NAME
#define IREE_OCL_TRACE_IMPL_GET_VARIADIC_(args) \
  IREE_OCL_TRACE_IMPL_GET_VARIADIC_HELPER_ args

#else

#define IREE_OCL_TRACE_ZONE_BEGIN(zone_id) \
  iree_ocl_zone_id_t zone_id = 0;          \
  (void)zone_id;
#define IREE_OCL_TRACE_ZONE_BEGIN_NAMED(zone_id, name_literal) \
  IREE_OCL_TRACE_ZONE_BEGIN(zone_id)
#define IREE_OCL_TRACE_ZONE_BEGIN_NAMED_COLORED(zone_id, name_literal, color) \
  IREE_OCL_TRACE_ZONE_BEGIN(zone_id)
#define IREE_OCL_TRACE_ZONE_END(zone_id) (void)(zone_id)

#define IREE_OCL_TRACE_ZONE_APPEND_VALUE_I64(zone_id, value)
#define IREE_OCL_TRACE_ZONE_APPEND_TEXT_LITERAL(zone_id, value_literal)
#define IREE_OCL_TRACE_ZONE_APPEND_TEXT_DYNAMIC(zone_id, ...)
...
// IREE_OCL_TRACE_ALLOC_NAMED/IREE_OCL_TRACE_FREE_NAMED pair.
#define IREE_OCL_TRACE_ALLOC_NAMED(name_literal, ptr, size)       \
...
// IREE_OCL_TRACE_ALLOC_NAMED.
#define IREE_OCL_TRACE_FREE_NAMED(name_literal, ptr)              \
...
#define IREE_OCL_TRACE_MESSAGE_LITERAL(level, value_literal)           \
  IREE_OCL_TRACE_MESSAGE_COLORED(IREE_OCL_TRACE_MESSAGE_LEVEL_##level, \
                                 (value_literal))

// Logs a message with the given color to the trace.
// Standard colors are defined as IREE_OCL_TRACE_MESSAGE_LEVEL_* values.
// The message text must be a compile-time string literal.
#define IREE_OCL_TRACE_MESSAGE_LITERAL_COLORED(color, value_literal) \
  iree_hal_amdgpu_device_trace_message_literal(                      \
      IREE_OCL_TRACE_BUFFER(), (color),                              \
      (iree_hal_amdgpu_trace_string_literal_ptr_t)(value_literal))
...
#define IREE_OCL_TRACE_MESSAGE_DYNAMIC(level, value, value_length)             \
  IREE_OCL_TRACE_MESSAGE_DYNAMIC_COLORED(IREE_OCL_TRACE_MESSAGE_LEVEL_##level, \
                                         (value), (value_length))

// Logs a dynamically-allocated message with the given color to the trace.
// Standard colors are defined as IREE_OCL_TRACE_MESSAGE_LEVEL_* values.
// The string |value| will be copied into the trace buffer.
#define IREE_OCL_TRACE_MESSAGE_DYNAMIC_COLORED(color, value, value_length) \
  iree_hal_amdgpu_device_trace_message_dynamic(                            \
      IREE_OCL_TRACE_BUFFER(), (color), (value), (value_length))
