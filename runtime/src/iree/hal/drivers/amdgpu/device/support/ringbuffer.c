...
bool iree_hal_amdgpu_device_ringbuffer_uint64_try_enqueue(
  ...
  uint32_t size = iree_hal_amdgpu_device_atomic_load_explicit(
  ...
  while (!iree_hal_amdgpu_device_atomic_compare_exchange_weak_explicit(
      &ringbuffer->size, &size, size + 1u,
      iree_hal_amdgpu_device_memory_order_seq_cst,
      iree_hal_amdgpu_device_memory_order_seq_cst,
      iree_hal_amdgpu_device_memory_scope_device)) {
    if (size == ringbuffer->capacity) {
  ...
  while (true) {
    ...
    uint32_t write_index = iree_hal_amdgpu_device_atomic_load_explicit(
        &ringbuffer->write_index, iree_hal_amdgpu_device_memory_order_seq_cst,
        iree_hal_amdgpu_device_memory_scope_device);
    while (!iree_hal_amdgpu_device_atomic_compare_exchange_weak_explicit(
...
iree_hal_amdgpu_device_ringbuffer_uint64_try_dequeue(
  ...
  while (true) {
    ...
    uint32_t read_index = iree_hal_amdgpu_device_atomic_load_explicit(
        &ringbuffer->read_index, iree_hal_amdgpu_device_memory_order_seq_cst,
        iree_hal_amdgpu_device_memory_scope_device);
    while (!iree_hal_amdgpu_device_atomic_compare_exchange_weak_explicit(
