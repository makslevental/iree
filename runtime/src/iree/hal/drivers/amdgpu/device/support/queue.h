...
#define iree_hsa_make_packet_header(type, is_barrier, scacquire_fence_scope, \
  ...
  (((type) << HSA_PACKET_HEADER_TYPE) |                                      \
   ((is_barrier ? 1 : 0) << HSA_PACKET_HEADER_BARRIER) |                     \
   ((scacquire_fence_scope) << HSA_PACKET_HEADER_SCACQUIRE_FENCE_SCOPE) |    \
   ((screlease_fence_scope) << HSA_PACKET_HEADER_SCRELEASE_FENCE_SCOPE))
...
extern uint64_t __ockl_hsa_queue_load_read_index(
    const IREE_OCL_GLOBAL iree_hsa_queue_t* queue,
    iree_hal_amdgpu_device_memory_order_t mem_order);
extern uint64_t __ockl_hsa_queue_load_write_index(
    const IREE_OCL_GLOBAL iree_hsa_queue_t* queue,
    iree_hal_amdgpu_device_memory_order_t mem_order);
extern uint64_t __ockl_hsa_queue_add_write_index(
    IREE_OCL_GLOBAL iree_hsa_queue_t* queue, uint64_t value,
    iree_hal_amdgpu_device_memory_order_t mem_order);
extern uint64_t __ockl_hsa_queue_cas_write_index(
    IREE_OCL_GLOBAL iree_hsa_queue_t* queue, uint64_t expected, uint64_t value,
    iree_hal_amdgpu_device_memory_order_t mem_order);
